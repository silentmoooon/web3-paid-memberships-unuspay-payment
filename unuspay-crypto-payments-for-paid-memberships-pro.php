<?php
    ...
    if (! function_exists('unuscpmp_pmp_gateway_load')) {
        ...
        function unuscpmp_pmp_gateway_load()
        {
            ...
            add_action('init', ['PMProGateway_unuspay', 'init']);
            ...
            add_filter('plugin_row_meta', ['PMProGateway_unuspay', 'plugin_row_meta'], 10, 2);
            ...
            class PMProGateway_unuspay extends PMProGateway
            {
                ...
                {
                    add_filter('pmpro_gateways', ['PMProGateway_unuspay', 'pmpro_gateways']);

                    add_filter('pmpro_payment_options', ['PMProGateway_unuspay', 'pmpro_payment_options']);
                    add_filter('pmpro_payment_option_fields', ['PMProGateway_unuspay', 'pmpro_payment_option_fields'], 10, 2);

                    $gateway = pmpro_getGateway();
                    if ($gateway == "unuspay") {
                        add_filter('pmpro_include_billing_address_fields', '__return_false');
                        add_filter('pmpro_include_payment_information_fields', '__return_false');
                        add_filter('pmpro_required_billing_fields', ['PMProGateway_unuspay', 'pmpro_required_billing_fields']);
                        add_filter('pmpro_checkout_before_change_membership_level', ['PMProGateway_unuspay', 'pmpro_checkout_before_change_membership_level'], 1, 2);
                    }
                }
                ...
                public static function pmpro_gateways($gateways)
                {
                    if (empty($gateways['unuspay'])) {
                        $gateways = array_slice($gateways, 0, 1) + ["unuspay" => esc_html__('UnusPay', 'unuspay-crypto-payments-for-paid-memberships-pro')] + array_slice($gateways, 1);
                    }
               ...
               <p style="margin-top: 20px;"><a href="https://dapp.unuspay.com/dashboard/" target="_blank">Get Started</a></p>
                ...
                <?php
                    ...
                    function pmp_custom_scripts()
                    {
                        // 仅在 EDD 结账页面加载
                        //if (pmp_is_checkout()) {
                        wp_register_script('UNUSCPMP_PMP_WIDGETS', plugin_dir_url(__FILE__) . 'assets/js/widgets.bundle.js', [], '1.0', true);
